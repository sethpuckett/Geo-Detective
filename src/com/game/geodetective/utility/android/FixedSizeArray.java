...
package com.game.geodetective.utility.android;
...
public class FixedSizeArray<T> extends AllocationGuard {
    protected static StringBuffer _tag = new StringBuffer("FixedSizeArray");	
    ...
    public final void add(T object) {
        if (mCount < mContents.length) {
        	...
        	Logger.e(_tag, "Cannot add; array is full");
    }
    
    /**
     * Adds to this array all the elements that are contained in the specified array
     */
    public void addAll(FixedSizeArray<T> ar1) {
    	int count = ar1.getCount();
    ...
    /**
     * Removes from this array all the elements that are contained in the specified array
     */
    public void removeAll(FixedSizeArray<T> ar1, boolean ignoreComparator) {
    	int count = ar1.getCount();
    	for (int i = 0; i < count; i++) {
    		remove(ar1.get(i), ignoreComparator);
    	}
    }
